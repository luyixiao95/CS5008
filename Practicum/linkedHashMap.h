...
//Use typedef to create the fundemental node struct
typedef struct node{
	int value;
	int key;
	struct node* next;
...
//build the bucket struct
typedef struct bucket{
	node_t* old;
	node_t* new;
}bucket_t;

//The easiest hashkey function I chose (Using remainder)
...
//The linkedHashMap type that contains header, which is the
...
//makeLinkedHashMap funciton make a new linkedhashmap
...
//addtoLinkList adding the new element to the linkedlist part.
...
void swapNearTwo(linkedHashMap_t* lhm, int key1){



	node_t* node1 = search(lhm, key1);
	node_t* node2 = node1->after;
...
void swap(linkedHashMap_t* lhm, int key1, int key2){
	...
	if(hashkey2 == hashkey1+ 1){

	swapNearTwo(lhm, key1);
	return;
...
//Bad idea of sorting! But have limited time to revise sadly.
